    ...
    <div className="w-full max-w-full overflow-x-hidden space-y-10 bg-white text-slate-900 p-8 md:p-10 min-h-screen">
            ...
            <div className="relative">
              <select
                ...
                className={`w-full py-3.5 px-4 pr-10 text-sm sm:text-base font-bold rounded-xl border transition-all cursor-pointer appearance-none shadow-2xs ${
                  period !== 'ALL'
                    ? 'bg-indigo-50/50 border-indigo-300 text-indigo-950 ring-2 ring-indigo-500/20'
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-6 w-full min-w-0">
        ...
        >
          ...
          <div className="text-3xl sm:text-4xl font-black text-indigo-600 tracking-tight truncate">
        ...
        <div className="bg-white p-7 rounded-2xl border border-slate-200/70 shadow-xs hover:shadow-md hover:border-slate-300 transition-all flex flex-col justify-between min-h-[175px] space-y-4 min-w-0">
          ...
          <span className="text-xs font-bold text-indigo-600">Approved Leave Requests</span>
        </div>
              ...
              <div className="w-full h-56 flex items-end gap-4 pt-8 pb-3 px-3 border-b border-slate-100 relative">
                ...
                {barChartData.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-xs font-bold transition whitespace-nowrap ${isHovered ? 'text-indigo-700 scale-105' : 'text-indigo-600 opacity-90'}`}>
                        ₹{(item.totalCost / 1000).toFixed(0)}k
                      </span>
                      <div
                        className={`w-full rounded-t-xl transition-all duration-300 shadow-xs ${
                          isHovered
                            ? 'bg-gradient-to-t from-indigo-700 to-indigo-500 ring-2 ring-indigo-400 ring-offset-1'
                            : 'bg-gradient-to-t from-indigo-600 to-indigo-400 hover:from-indigo-700 hover:to-indigo-500'
                        }`}
                        style={{ height: `${heightPct}%` }}
                      ></div>
                      <span className="text-xs font-bold text-slate-700 truncate w-full text-center mt-1 group-hover:text-indigo-600">
                        {item.department}
                      </span>
                    </div>
                  );
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {barChartData.items.slice(0, 4).map((item) => (
                  <div
                    key={item.department}
                    onClick={() => setSelectedDeptDetail(item)}
                    className="flex justify-between items-center bg-slate-50/80 hover:bg-indigo-50/60 p-3 rounded-xl cursor-pointer transition border border-transparent hover:border-indigo-100"
                  >
              ...
              <tbody className="divide-y divide-slate-100/80 font-medium">
                {!data?.timeOffTable || data.timeOffTable.length === 0 ? (
                  ...
                  data.timeOffTable.map((row) => (
                    <tr key={row.type} className="hover:bg-indigo-50/50 transition">
                      <td className="py-3.5 px-3 font-bold text-slate-800 leading-tight">{row.type}</td>
                      <td className="py-3.5 px-3 text-center font-bold text-indigo-600 whitespace-nowrap">{row.approvedDays}d</td>
                ...
                {!data?.departmentOverview || data.departmentOverview.length === 0 ? (
                  ...
                  data.departmentOverview.map((row) => (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-3 text-center font-bold text-indigo-600 whitespace-nowrap">{row.headcount}</td>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/70 shadow-xs hover:shadow-sm transition-all p-7 space-y-6 min-h-[380px] flex flex-col justify-between min-w-0 overflow-hidden">
          ...
          <div className="space-y-4 text-sm text-slate-700">
            ...
            <div className="flex justify-between border-b border-slate-100 pb-2">
              ...
              <span className="font-bold text-indigo-600">{data?.modelsSummary?.payslipsCount || 0} Payslips</span>
            </div>
            <div className="flex justify-between border-b border-slate-100 pb-2">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-indigo-500">{data?.modelsSummary?.timeoffRequestsCount || 0} Requests</span>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white rounded-2xl p-7 max-w-md w-full shadow-2xl space-y-6 border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <span className="text-xs font-extrabold uppercase tracking-widest text-indigo-600">Department Inspection</span>
            ...
            <div className="space-y-4 text-sm">
              <div className="p-4 bg-indigo-50/70 rounded-xl border border-indigo-100 flex justify-between items-center">
                <span className="font-semibold text-indigo-900">Total Department Salary:</span>
                <span className="text-xl font-black text-indigo-700">₹{selectedDeptDetail.totalCost.toLocaleString()}</span>
              </div>
